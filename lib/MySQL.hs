...
connInfo = defaultConnectInfo { connectHost = "127.0.0.1", connectUser = "test", connectPassword = "test", connectDatabase = "testdb" }
...
selectPersonById id = do
  ...
  case sqlResult of
    ...
    Nothing -> throwError err404 { errBody = JSON.encode ("Person with ID not found." :: String) }
...
createPerson person = do
  ...
  case attemptCreate of
    ...
    _           -> throwError err503 { errBody = JSON.encode ("Could not create Person." :: String) }
